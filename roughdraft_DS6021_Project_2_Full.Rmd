...
```{r}
metrics <- data.frame(c('BIC', 'Mallows CP', 'Adjusted R squared'), 
           c(which.min(summary(allreg_red)$bic), which.min(summary(allreg_red)$cp), which.max(summary(allreg_red)$adjr2)))
colnames(metrics) <- c('Critera', 'Number of predictors selected')
metrics
```
...
[comment]: <> (log(pi(quality)/1+pi(quality)) = 285.10221 + (0.72351)alcohol + (-2.82651)volatile.acidity + (3.23950)sulphates +  (0.36775)fixed.acidity + (-299.78128)density + (0.17416)residual.sugar + (-7.54886)chlorides)  
...
$H_{0} : \beta_{citic.acid} = \beta_{total.sulfur.dioxide} = \beta_{pH} = 0$  

versus the alternative hypothesis: 

[comment]: <> (Ha:at least one of the coefficients is not zero.)  
$H_{a} : \exists x \in \{citric.acid, total.sulfur.dioxide, pH\} s.t. \beta_{x} \neq 0$
...
The resulting VIFs suggests that none of the variables are linearly dependent on one another, and we suspect that pH and citric.acid are not significant predictors for classifying red wine as "low" or "high". To validate dropping both of these predictors from the full model, we compare our 8-predictor model to the full (10-predictor) model. 
...
```{r}
metrics <- data.frame(c('BIC', 'Mallows CP', 'Adjusted R squared'), 
           c(which.min(summary(allreg_white)$bic), which.min(summary(allreg_white)$cp), which.max(summary(allreg_white)$adjr2)))
colnames(metrics) <- c('Critera', 'Number of predictors selected')
metrics
```
...
Alcohol in white wines is highly correlated with both density and residual sugar as well, so multicolinearity is expected to be present in this model. Therefore, it is likely that alcohol is truly a good predictor, but multicolinearity is causing it to fail the wald test. Before addressing this issue, we should test whether this model is better than the intercept, and better than the model with all predictors.  
...
The code has produced a $\chi^{2}$ test statistic of 757.2668, and a corresponding p-value of $\approx 0$.  
...
We get a $\chi^{2}$ test statistic of 15.15094 and a corresponding p-value of $\approx 0.001$.  
Therefore, we reject the null hypothesis and conclude that at least one predictor is significant.  
This means that we prefer the full model over the seven predictor model, and should add in the rest of the predictors.  

The output shows that the full model has wald tests that fail, and from earlier correlation plots we know that many of the predictors are highly correlated with one another, so we must assess and fix multicolinearity.  
...
Both residual sugar and density have a VIF higher than 10, meaning that multicolinearity is present, so we should take them out of the model. We ran the hypothesis test for removing the two predictors, and found that the test still prefered the full model. However, we decided to ignore this test for the sake of producing a model that does not contain significant correlation among the predictors.  
...
The $\chi^{2}$ test statistic is 3.95 and has a corresponding p value of 0.27.  
...
```{r}
print('Confusion matrix for the 7 predictor model')
...
$\log{\big(\frac{\hat\pi_{quality\_cat}}{1 - \hat\pi_{quality\_cat}}\big)} = -11.30223 - 3.3808*x_{volatile.acidity} - 18.68852*x_{chlorides} + 0.89333*x_{pH} + 1.10611*x_{sulphates} + 0.76496*x_{alcohol}$
